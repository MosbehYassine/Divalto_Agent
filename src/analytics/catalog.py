...
from core.settings import AUTO_GENERATE_SEMANTIC_FROM_SQLITE, SEMANTIC_MODEL_DIR, SQLITE_MOCK_DB_PATH


def default_semantic_dir() -> Path:
    """Default: agent_project/generated_olap next to this package."""
    return Path(__file__).resolve().parents[2] / "generated_olap"
...
def ensure_semantic_from_sqlite(semantic_dir: Path, sqlite_path: Path) -> dict[str, Any] | None:
    ...
    from analytics.auto_olap_pipeline import run_auto_modeling
