...
def _lookup_hint_is_noisy(hint: str) -> bool:
    ...
    if any(m in low for m in noise_markers):
        return True
    if re.search(r"\bqui\s+", low):
        return True
    if re.search(r"\b(plus|moins|meilleur|top|principal|premier)\s*$", low):
        return True
    if re.search(r"\b(achete|achète|achats|genere|génère|commande)\b", low):
        return True
    return False
...
def extract_lookup_hint(text: str) -> str:
    ...
    # Superlative / ranking questions are not name lookups (e.g. « client qui achète le plus »).
    try:
        ...
        if (
            is_client_sales_ranking_query(raw)
            or is_client_count_query(raw)
            or is_dimensional_breakdown_query(raw)
        ):
            return ""
    except ImportError:
        pass
