...
from core.intent_contract import (
    ...
    breakdown_metric_from_query,
    client_ranking_sort_metric,
    decompose_query,
    extract_ca_threshold_from_query,
    infer_intent,
    is_client_ca_threshold_query,
...
from phase4.prefunctions import extract_lookup_hint, normalize_text
from core.prompt_templates import load_prompt_template
from llm.rag_retriever import retrieve_context
from routing.semantic_router import get_available_actions, select_best_action
...
def _is_client_count_query(user_query: str) -> bool:
    return is_client_count_query(user_query)
...
def mock_plan(user_query: str) -> list:
    ...
    if "consulter_indicateurs_analytiques" in available and is_client_ca_threshold_query(user_query):
        threshold = extract_ca_threshold_from_query(user_query)
    ...
    if "classement_clients" in available and is_client_sales_ranking_query(user_query):
        return [
            {
                "step": 0,
                ...
                "limit": 5,
                "order": "desc",
                "sortBy": client_ranking_sort_metric(user_query),
            }
        ]

    if "consulter_indicateurs_analytiques" in available and is_dimensional_breakdown_query(user_query):
        return [
            {
                "step": 0,
                "action": "consulter_indicateurs_analytiques",
                "metric": breakdown_metric_from_query(user_query),
                "groupBy": breakdown_group_by_from_query(user_query),
            }
        ]
...
def apply_dimensional_breakdown_plan_override(user_query: str, plan_steps: list[dict]) -> list[dict]:
    """Force indicateurs analytiques for « CA par ville », etc."""
    if not is_dimensional_breakdown_query(user_query):
        return plan_steps
    if "consulter_indicateurs_analytiques" not in get_available_actions():
        return plan_steps
    return [
        {
            "step": 0,
...
def apply_client_ca_threshold_plan_override(user_query: str, plan_steps: list[dict]) -> list[dict]:
    """Clients whose CA exceeds a numeric threshold (liste + supérieur à X)."""
    if not is_client_ca_threshold_query(user_query):
        return plan_steps
    if "consulter_indicateurs_analytiques" not in get_available_actions():
        return plan_steps
    threshold = extract_ca_threshold_from_query(user_query)
...
def apply_client_ranking_plan_override(user_query: str, plan_steps: list[dict]) -> list[dict]:
    """Force classement_clients for top-client-by-sales questions."""
    if not is_client_sales_ranking_query(user_query):
        return plan_steps
    if "classement_clients" not in get_available_actions():
        return plan_steps
    return [
        {
            "step": 0,
...
def enrich_plan_lookup_hints(user_query: str, plan_steps: list[dict]) -> list[dict]:
    ...
    if is_client_sales_ranking_query(user_query):
        return plan_steps
    if is_client_ca_threshold_query(user_query):
        return plan_steps
...
def plan(user_query: str, use_mock: bool = True) -> list:
    ...
    if bypass is not None:
        ...
        try:
            ...
            safe_print(f"[Planner] Real planner failed, fallback to mock: {exc}")
            result = mock_plan(user_query)

    result = apply_client_count_plan_override(user_query, result)
    result = apply_client_ca_threshold_plan_override(user_query, result)
    result = apply_client_ranking_plan_override(user_query, result)
    result = apply_dimensional_breakdown_plan_override(user_query, result)
    result = enrich_plan_lookup_hints(user_query, result)

    safe_print(f"[Planner] Query: '{user_query}'")
    safe_print(f"[Planner] Plan: {json.dumps(result, ensure_ascii=False)}")
    return result
